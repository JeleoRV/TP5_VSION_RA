...
void drawSnowMan(float axisSize) {
   // On va dessiner en blanc
   glColor3f(1.0f, 1.0f, 1.0f);
   
   // Corps du bonhomme de neige : une sphere
   glTranslatef(0.0f ,0.75f*axisSize, 0.0f);
   glutSolidSphere(0.75f*axisSize,20,20);
   
   // Tete de bonhomme : une sphere
   glTranslatef(0.0f, 1.0f*axisSize, 0.0f);
   glutSolidSphere(0.25f*axisSize,20,20);
   
   // Yeux du bonhomme de neige : deux spheres noires
   glPushMatrix();
   glColor3f(0.0f,0.0f,0.0f);
   glTranslatef(0.05f*axisSize, 0.10f*axisSize, 0.18f*axisSize);
   glutSolidSphere(0.05f*axisSize,10,10);
   glTranslatef(-0.1f*axisSize, 0.0f, 0.0f);
   glutSolidSphere(0.05f*axisSize,10,10);
   glPopMatrix();
   ...
   glutSolidCone(0.08f*axisSize,0.5f*axisSize,10,2);
}
...
void ArUco::drawScene() {
   ...
   for (unsigned int m=0;m<m_Markers.size();m++)
   {
	  ...
	  glRotatef(90,1,0,0); // Pour remettre les bonshommes debout
	  drawSnowMan(0.8*m_MarkerSize);
